...
class Runner(object):
	...
	def add_model(self, model, score_func):
		...
		if   model_name.lower()	== 'compgcn_conve': 	model = ConvE(self.p, self.p.num_ent, self.p.num_rel)
	...
	def evaluate(self, split, epoch):
		...
		self.logger.info('[Epoch {} {}]: MR: {:.5}'.format(epoch, split, results['mr']))
		self.logger.info('[Epoch {} {}]: Hits@1: {:.5}, Hits@3: {:.5}, Hits@10: {:.5}\n'.format(epoch, split, results['hits@1'], results['hits@3'], results['hits@10']))
	...
	def fit(self):
		...
		self.best_val_hits1 = 0.
		self.best_val_hits3 = 0.
		self.best_val_hits10 = 0.
		self.best_test_hits1 = 0.
		self.best_test_hits3 = 0.
		self.best_test_hits10 = 0.
		save_path = os.path.join('./torch_saved', self.p.name)

		if self.p.restore:
			model_path = os.path.join('./torch_saved', self.p.restore_model)
...
if __name__ == '__main__':
	...
	parser.add_argument('-batch',           dest='batch_size',      default=500,    type=int,       help='Batch size')
	...
	parser.add_argument('-restore_model',         dest='restore_model', default=None , type=str,            help='Restore from the previously saved model')
	...
	parser.add_argument('-input_drop',	dest='input_drop', 	default=0.2,  	type=float,	help='Dropout to use in GCN Layer')
	parser.add_argument('-hidden_drop',  	dest='hidden_drop', 	default=0.3,  	type=float,	help='Dropout after GCN')

	# ConvE specific hyperparameters
	parser.add_argument('-hidden_size', dest='hidden_size', default=9728, type=int,
